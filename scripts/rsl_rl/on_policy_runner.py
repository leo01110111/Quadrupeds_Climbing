...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        self.video_folder = os.path.join(log_dir, "videos", "train") if log_dir else None
        self.video_interval = train_cfg.get("video_interval", 2000)  # Default to 2000 steps
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        # Track global steps for video logging
        global_step = 0
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    global_step += self.env.num_envs  # Increment global step per environment step
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if self.logger_type == "wandb" and self.video_folder and locs["global_step"] % self.video_interval == 0:
            import wandb
            video_files = glob.glob(os.path.join(self.video_folder, "*.mp4"))
            for video_file in video_files:
                wandb.log({
                    "video": wandb.Video(video_file, fps=30, format="mp4"),
                    "global_step": locs["global_step"]
                })
                # Optionally remove video file to save disk space
                os.remove(video_file)  # Uncomment to delete videos after logging
